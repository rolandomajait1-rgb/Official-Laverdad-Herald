...
const RelatedCard = ({ article, onClick }) => (
  <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col group cursor-pointer hover:shadow-md transition-all" onClick={onClick}>
    ...
    <div className="p-5 flex flex-col grow">
      ...
      <h3 className="text-lg font-serif font-bold text-gray-900 mb-3 leading-tight group-hover:text-yellow-800 transition-colors line-clamp-2 text-left">
        {article.title}
      </h3>
      {article.excerpt && (
    ...
    <div className="sm:w-1/3 relative overflow-hidden h-48 sm:h-auto">
      ...
      {(onEdit || onDelete) && (
        <div className="absolute top-2 right-2 flex gap-1">
          {onEdit && (
            <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="p-1.5 bg-yellow-500 text-white rounded shadow hover:bg-yellow-600">
              <Pencil size={14}/>
            </button>
          )}
          {onDelete && (
    ...
    <div className="p-5 sm:w-2/3 flex flex-col justify-between">
      <div>
        <div className="flex justify-between items-center mb-2">
          <span className="bg-yellow-100 text-yellow-800 text-xs font-bold px-2 py-0.5 rounded uppercase">{article.category}</span>
          <span className="text-gray-400 text-xs flex items-center gap-1 text-left"><Calendar size={12}/> {article.date}</span>
        </div>
        <h3 className="text-xl font-serif font-bold text-gray-900 mb-2 group-hover:text-yellow-800 transition-colors text-left">
          {article.title}
        </h3>
...
const MostViewedCard = ({ article, onClick }) => (
  <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-3 group cursor-pointer hover:border-yellow-300 transition-colors" onClick={onClick}>
    <div className="flex justify-between items-center mb-2">
      <span className="text-[10px] text-gray-400 text-left flex items-center gap-1"><Calendar size={10}/> {article.date}</span>
      <span className="text-[10px] font-bold text-yellow-600 bg-yellow-50 px-1.5 py-0.5 rounded border border-yellow-100">{article.category}</span>
    </div>
    <h4 className="text-sm font-bold text-gray-800 leading-snug mb-2 group-hover:text-yellow-700 text-left">
      {article.title}
    </h4>
    ...
    const fetchFeaturesArticles = async () => {
      ...
      try {
        ...
        const relatedResponse = await axios.get('/api/articles', {
          params: { category: 'features', limit: 12 }
        });
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <HeaderLink />
        ...
        <div className="bg-news-bg bg-cover bg-center h-20" style={{
                  backgroundImage: `linear-gradient(to right, #FFA500 20%, rgba(255,165,0,0.2)), url('/images/bg.jpg' )`
                }}>
        ...
        {loading ? (
              ...
              <div className="lg:col-span-2">
                <div className="bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden flex flex-col h-144">
                  ...
                  <div className="p-3 flex flex-col grow">
                    <div className="flex justify-between items-start mb-3">
                      <div className="bg-linear-to-r from-yellow-100 to-yellow-200 text-xs font-semibold uppercase px-3 py-2 rounded cursor-pointer hover:opacity-80 h-10 w-24 animate-pulse"></div>
              ...
              <div className="lg:col-span-1 flex flex-col gap-8">
                {Array.from({ length: 2 }).map((_, index) => (
                  <div key={index} className="bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden flex flex-col max-w-full">
                    ...
                    <div className="p-3 flex flex-col grow">
                      <div className="flex justify-between items-start mb-3">
                        <div className="bg-linear-to-r from-yellow-100 to-yellow-200 text-xs font-semibold uppercase px-3 py-2 rounded cursor-pointer hover:opacity-80 h-8 w-22 animate-pulse"></div>
                  ...
                  {Array.from({ length: 4 }).map((_, index) => (
                    <div key={index} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col sm:flex-row mb-4">
                      ...
                      <div className="p-5 sm:w-2/3 flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-center mb-2">
                            <div className="bg-linear-to-r from-yellow-100 to-yellow-200 text-xs font-bold px-2 py-0.5 rounded uppercase h-6 w-16 animate-pulse"></div>
                  ...
                  {Array.from({ length: 5 }).map((_, index) => (
                    <div key={index} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-3">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="bg-linear-to-r from-yellow-100 to-yellow-200 text-[10px] font-bold px-1.5 py-0.5 rounded border h-4 w-12 animate-pulse"></div>
                      </div>
          ...
          <>
              ...
              <div className="lg:col-span-2">
                {articles[0] && (
                  <ArticleCard
                    ...
                    category={articles[0].categories && articles[0].categories.length > 0 ? articles[0].categories[0].name : 'Features'}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-6 border-b-2 border-yellow-600 pb-2">Latests</h2>
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-6 border-b-2 border-yellow-600 pb-2">Most Viewed</h2>
                <div className="space-y-4">
                    ...
                    {relatedArticles.map((relatedArticle) => (
                      <RelatedCard 
                        ...
                        article={{
                          ...
                          category: relatedArticle.categories && relatedArticle.categories.length > 0 ? relatedArticle.categories[0].name : 'Features',
